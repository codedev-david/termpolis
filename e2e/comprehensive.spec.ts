...
test.beforeAll(async () => {
  ...
  app = await electron.launch({
    args: e2eLaunchArgs('comprehensive'),
...
test.describe.serial('3. Terminal Creation', () => {
  ...
  test('3.12 terminal rendered', async () => {
    // Terminal was created â€” verify xterm is rendering
    await page.waitForTimeout(1000)
    await ss('3.12-terminal-rendered')
    // Non-blocking â€” terminal is visible if we got to this point
  })
...
test.describe.serial('8. Workflows overlay', () => {
  test('8.1 opens from sidebar', async () => {
    // Retired toolbar icon â†’ permanent sidebar section's New Workflow button.
