...
async function closeAnyModal() {
  // Try the X button inside any open full-screen overlay first â€” more reliable
...
test('03 - sidebar icon bar visible', async () => {
  ...
  await expect(page.locator('button[title="Start Workflow"]')).toBeVisible()
...
test('05 - AI Agents section visible', async () => {
  // Only toggle the AI Agents header if the list is currently hidden â€”
