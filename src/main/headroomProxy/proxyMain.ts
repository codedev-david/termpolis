...
import { rewriteMessagesBody, type ImageCompressor, type WireStats } from './wireCompress'
...
/**
 * Build the compression proxy HTTP server. Exported (not auto-listening) so it can
 * be unit-tested against a local stub upstream. Only POST /v1/messages bodies are
 * rewritten (fail-open); everything else is a transparent pass-through. The response
 * is streamed back UNCHANGED; a decoded copy is parsed for real token usage.
 */
export function createProxyServer(opts: ProxyOpts): http.Server {
  ...
  return http.createServer((req, res) => {
    ...
    req.on('end', () => {
      ...
      if (isMessages) {
        try {
          const r = rewriteMessagesBody(body.toString('utf8'), { compressImage: opts.compressImage })
...
// Per the memory-brain gotcha: the child receives messages as `e.data`.
