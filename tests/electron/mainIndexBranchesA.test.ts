...
const M = vi.hoisted(() => ({
  ...
  execSync: vi.fn(),
  execFileSync: vi.fn(),
  // v1.47.1: git spawns off the main thread, so the callback flavours are what production reaches.
  // Both delegate to their sync twin, which is what the tests below still stub.
