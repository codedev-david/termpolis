...
- [ ] **Step 2: Run to verify failure.** `npx vitest run tests/electron/remoteIdentityStore.test.ts` → FAIL (module not found).
- [ ] **Step 3: Implement.** File `remote-identity-key` in userData, written through `writeSecret` so it is DPAPI/Keychain-encrypted. Validate `/^[0-9a-f]{64}$/` on read; anything else re-mints. The public key is always derived, never stored — a stored pair can disagree with itself, and a wrong public key means every device pairs against an identity this desktop cannot prove.
- [ ] **Step 4: Run to verify pass.**
- [ ] **Step 5: Commit.** `feat(remote): persist the desktop's X25519 identity`
...
- [ ] **Step 3: Implement.** Validation is the point: this file decides what a phone is allowed to do, so a capability that is `undefined` on disk must read as `false`, never as truthy-by-omission. Unknown keys are dropped.
- [ ] **Step 4: Run to verify pass.**
- [ ] **Step 5: Commit.** `feat(remote): persist paired devices`
...
- [ ] **Step 1: Write the failing tests** — defaults are `{ enabled: false, relayUrl: DEFAULT_RELAY_URL }`; a missing file yields defaults; a partial patch merges; a relay URL that is not `ws:`/`wss:` is rejected and the previous value kept.
- [ ] **Step 2: Run to verify failure.**
- [ ] **Step 3: Implement.** Reject any relay URL that is not `ws:`/`wss:` — an `http:` URL would fail at dial time with a message that names neither the setting nor the reason.
- [ ] **Step 4: Run to verify pass.**
- [ ] **Step 5: Commit.** `feat(remote): remote settings with an off-by-default switch`
...
- [ ] **Step 4: Run to verify pass.** Also re-run `remoteBridgeEntry.test.ts` and `remoteOutputFanout.test.ts`.
- [ ] **Step 5: Commit.** `feat(remote): pump output only for subscribed terminals`
...
- [ ] **Step 1: Write the failing test** — `createRemoteBridgeTransport(path, relayUrl)` forks with `env.TERMPOLIS_RELAY_URL === relayUrl`, and omitting the URL leaves the parent env untouched.
- [ ] **Step 2: Run to verify failure.**
- [ ] **Step 3: Implement.** `utilityProcess.fork(path, [], { serviceName, env: { ...process.env, TERMPOLIS_RELAY_URL: relayUrl } })`. `entry.ts` already reads that variable; without this the setting is inert and a local relay is untestable.
- [ ] **Step 4: Run to verify pass.**
- [ ] **Step 5: Commit.** `feat(remote): pass the relay URL to the bridge process`
...
- [ ] **Step 1: Write the failing tests** — each handler invokes its host function and returns `ok(...)`; `remote:set-capabilities` rejects a payload whose flags are not booleans; preload exposes each method and `onRemoteEvent` returns an unsubscribe function.
- [ ] **Step 2: Run to verify failure.**
- [ ] **Step 3: Implement.** In `index.ts`, after `mcpServer = startMcpServer(...)`: await the bound port, then `startRemoteBridgeHost({...})` inside try/catch. In the PTY `onData` handler beside `appendOutput(terminalOutputBuffers, id, data)` add `noteTerminalOutput(id)`; beside `terminalOutputBuffers.delete(terminalId)` add `noteTerminalClosed(terminalId)`. Validate capability payloads at the IPC boundary — this is the one place a compromised renderer could grant itself `writeToTerminal`.
- [ ] **Step 4: Run to verify pass.**
- [ ] **Step 5: Commit.** `feat(remote): spawn the bridge from main and expose it over IPC`
...
- [ ] **Step 1: Write the failing tests** — renders nothing with no attached device; renders a phone icon and the count with one or more; clicking opens Settings on the `remote` tab via `setPendingSettingsTab`.
- [ ] **Step 2: Run to verify failure.**
- [ ] **Step 3: Implement.** A separate component rather than markup inside `TitleBar.tsx`, which is excluded from the coverage gate — logic put there is logic nothing tests. Spec §4.4: the user must be able to tell at a glance that a phone is attached.
- [ ] **Step 4: Run to verify pass.**
- [ ] **Step 5: Commit.** `feat(remote): show when a phone is attached`
...
- [ ] **Step 2:** `npx eslint <every touched file> --max-warnings 0`
- [ ] **Step 3:** Prepend `C:\Program Files\Git\cmd` to PATH, then `npm run test:coverage` — expect exit 0 and all four floors clear.
- [ ] **Step 4:** Manual smoke: enable Remote, pair with `node scripts/remote-test-client.cjs`, confirm the safety phrase matches on both sides, subscribe to a terminal, watch output arrive, revoke, confirm the socket closes.
