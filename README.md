...
- **Both halves of the wire, not just the reply (v1.34).** A tool call has two sides: the **result** that came back, and the **`tool_use` the agent itself wrote** — the 400-line file body in a `Write`, the giant `Edit` payload — which is re-read out of the prefix on *every* later turn. Earlier versions compressed only the first. Both are compressed now, and identifier fields (`file_path`, `command`, `pattern`, `url`) are excluded by name so a call never becomes ambiguous.
...
- **Measured, not asserted.** 4,183 real requests from this machine's own transcripts — **35.2 GB of request bodies** — were replayed through the compressor. At the shipped default: **61.0% of compressible tool text removed** (63.5% of tool results, 51.6% of `tool_use` payloads), **median request 61.4%**, and **93.8% of requests clear 50%**. At the Maximum tier: **72.3%**, with **99.9%** of requests clearing 50%. A [regression test](tests/electron/headroomFloorRealistic.test.ts) now fails the build if a threshold is ever loosened.
...
- **Prefix decay — opt-in, and honest about why.** On very long conversations the oldest half can be aged down to retrievable stubs. It is **off by default** because it is the one control that can *cost* money: shortening the prefix forces a re-cache (~1.15× the prefix), which needs roughly **39 more turns** to repay at typical volumes. The cutoff advances only at **doublings** (64 → 128 → 256 messages), so a 300-turn session pays at most a handful of breaks instead of one per turn.
